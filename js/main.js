...
(() => {
  ...
  const IMAGES = {
    ...
    // solutions
    's-thermoforming': A + 'solutions/thermoforming-clean.jpg',
    's-injection':     A + 'solutions/injection-clean.jpg',
    's-mold':          A + 'solutions/mold-clean.jpg',
    's-extrusion':     A + 'solutions/extrusion-clean.jpg',
    's-cnc':           A + 'solutions/cnc-clean.jpg',
    's-transport':     A + 'solutions/transport-clean.jpg',
    's-diwashing':     A + 'solutions/diwashing-clean.jpg',
    's-film':          A + 'solutions/film-clean.jpg',
    's-warehousing':   A + 'solutions/warehousing-clean.jpg',
    's-sourcing':      A + 'solutions/sourcing-clean.jpg',
    's-assembly':      A + 'solutions/assembly-clean.jpg'
  };
  ...
  function initLang() {
    document.querySelectorAll('.lang').forEach((g) => g.querySelectorAll('.lang__opt').forEach((o) => {
      o.addEventListener('click', () => {
        document.querySelectorAll('.lang__opt').forEach((x) => {
          const on = x.dataset.lang === o.dataset.lang;
  ...
  function initClientsCarousel() {
    ...
    // pos is a float mirror of scrollLeft: iOS rounds scrollLeft to integers, so
    // sub-pixel per-frame increments would truncate to zero if written directly
    let rafId = null, lastT = null, lastInput = 0, pos = 0;

    const userActive = () => performance.now() - lastInput <= IDLE_RESUME;
    const tick = (t) => {
    ...
    row.addEventListener('scroll', () => {
      if (!userActive()) return;              // our own writes; pos is authoritative
      const w = track.offsetWidth;
      if (w > 0 && row.scrollLeft >= w) row.scrollLeft -= w;
      pos = row.scrollLeft;                   // resume from where the user left off
    }, { passive: true });

    const start = () => { if (rafId === null) { lastT = null; pos = row.scrollLeft; rafId = requestAnimationFrame(tick); } };
    const stop = () => { if (rafId !== null) { cancelAnimationFrame(rafId); rafId = null; row.scrollLeft = 0; pos = 0; } };
